...
## Track 2 적용 결과 (2026-05-23, HYBRID lock)

codex 3 alternatives (STRICT / RELAXED / HYBRID) 적대적 검토 후 **HYBRID 채택** (owner 승인).

### Lock 재정의 (HYBRID)

본 lock은 STRICT 3-element-only 정의에서 **HYBRID primary-path + 허용 trust-continuation**으로 보강:

- **Primary path (반드시 fold 내)**: headline + sub + CTA (3 element)
- **허용 (선택적, CTA 이후 trust continuation)**: `HeroSecondaryCard` (VES/fixes/spend 지표) — CTA 직후 social-proof 보조
- **금지 (절대 fold 침범 금지)**: `hero-right` (ProductShot) — mobile에서 stack 상단 침범 시 lock 위반

### 적용 변경 (Step A+B+C)

**파일**: `app/globals.css` L2074-2094 `@media (max-width: 920px)` 블록

```css
.hero-eyebrow { display: none; }  /* Step A — context label 약화 */
.hero-chips   { display: none; }  /* Step B — 지원 툴 표시 CTA 이후로 */
.hero-right   { display: none; }  /* Step C — ProductShot mobile 숨김 (HYBRID 정의 충족) */
```

> 💡 Hero padding 압축 (option a) 미적용 — CTA bottom 467px 충분 여유, padding 유지로 시각 균형 보존.

### Playwright 재측정 (2026-05-23 22:14 KST)

| 메트릭 | 측정값 | 판정 |
|--------|--------|------|
| `headline.y / height` | 184 / 120 | ✅ |
| `sub.y / height` | 324 / 51.19 | ✅ |
| `cta.y / height` | 429.19 / 38 | ✅ |
| `cta_bottom` | 467.19 | ✅ (≤ 640 iOS Safari safety) |
| `hero-right (ProductShot)` | **null** (display:none) | ✅ HYBRID 금지 만족 |
| `hero-eyebrow` | **null** | ✅ |
| `hero-chips` | **null** | ✅ |
| `hero-secondary-card.y` | 467.19 (CTA 직후) | ✅ HYBRID 허용 (trust continuation) |
| `ticker.y / height` | 606.19 / 29 | ✅ HYBRID 허용 (CTA 이후 social-proof) |

### HYBRID Invariant (재정의)

본 lock 적용 후 다음 위반 시 즉시 alert:

1. **Primary path 누락**: `headline.height == 0` OR `sub.height == 0` OR `cta.height == 0` (3-element 중 하나 비가시)
2. **Primary path overflow**: `cta_bottom > 640` (iOS Safari address bar safety)
3. **금지 침범**: `hero-right (ProductShot)` 가 `display: none` 아님 (mobile ≤920px)

### 회귀 자동화 (Track 4 예고)

Track 2 layout 안정화 완료 → Track 4에서 Playwright `toHaveScreenshot` baseline + 위 3 invariant를 spec으로 lock. owner-reviewed baseline commit 의무 (first-green-baseline 안티패턴 회피).

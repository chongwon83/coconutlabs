...
### 2026-05-29 VES 분자 계약 완화 — "unknown ≠ partial" → "skip the unattributable, poison the errored" [S0]

- 문제: 라이브 VES 컬럼이 prod에서 전 행 "—"만 표시. 원인 진단(제로코드 프로브, `/tmp/ves_probe.py`): 2026-05-28 [P1] 수정([[위 763 엔트리 line 774]])이 "in-window 토큰 기여 세션의 cwd 중 **하나라도** git repo로 안 풀리면 분자 전체를 omit(unknown ≠ partial)"으로 못 박았는데, 실세계 모든 윈도우가 이 게이트에 걸림 — ① 습관적 워크스페이스 루트 실행(`Coconut Labs/`는 `web/`의 비-git 부모), ② `~`/`~/.claude` 세션, ③ 사후 제거된 worktree가 전부 poison. 즉 분자 자체는 풍부(week 48 / month 90, `git log` 교차확인)한데 단 한 개의 비-git cwd가 전부를 "—"로 붕괴. self-report 거부하고 git 실측 분자를 만든 [P1] 설계의 **strict 무결성 규칙이 역설적으로 분자를 영구 미표시로 만듦**.
- 버린 대안: (a) owner 신규 재업로드 프로브(제로코드) — 프로브가 구조적 poison 3종 전부 상존을 증명, cwd-less·비-git 세션이 사라지지 않는 한 어떤 업로드도 동일 omit → infeasible 판정. (b) read-side 보정(`lib/data.ts`에서 partial 허용) — 분자 산식을 collector 밖으로 누출 + schema 계약 흐림 + 한 곳 진실 원칙 위반. (c) strict 유지 + 사용자에게 "단일 repo 디렉토리에서만 실행하라" 안내 — solo 퍼널/제로설정 트러스트 약속 위반 + 습관적 실행 패턴과 충돌, 사실상 분자 포기.
- 핵심 트레이드오프: **계약을 의도적으로 완화**(원래 [P1]의 "unknown ≠ partial"을 부분 반전). 비-git/shallow/cwd-less cwd는 **SKIP**(귀속 불가 → 보수적 과소계상), 분자는 검증 가능한 git repo만으로 산출. None(unknown→omit)은 **3경우로 좁힘**: ① author email 부재, ② **어떤** cwd도 usable git repo로 안 풀림(검증 가능한 것 0), ③ **이미 resolve된** repo가 카운트 도중 git 에러(genuine error는 여전히 poison). 결과: VES 분자는 조작 불가능한 **보수적 하한**(분모/cost는 전 작업 포함 — 과소표현은 해도 절대 부풀리지 않음). 원래 [P1]이 지키려던 "unknown은 실제 0이 아니다"는 **중요한 곳에 보존**(검증 가능 repo 0 → None, resolved-repo 에러 → None) — 완전 폐기가 아니라 "귀속 불가(benign skip)"와 "resolved repo가 에러(anomalous poison)"를 분리.
- 선택 이유: 분자가 풍부히 존재(48/90)하는데 strict 게이트만으로 영구 "—"는 거짓 음성(false unknown). 보수적 하한은 self-report 조작 위협(원래 설계 동기)을 0으로 유지하면서 — 과소계상은 게임 불가 — 실측 분자를 드디어 표면화. 위험 3축은 원래 [P1]과 동일 3/3(분자 산식·4모듈·운영데이터로만 drift 표면화)이라 **`/codex` 교차 리뷰 의무** 그대로 적용.
- 강한 증거: 5파일 변경(gitcount.py 계약+tri-state, collect.py 게이트+센티넬, pyproject 0.1.0→0.1.1, test_gitcount +6, test_collector partial). **pytest 52 passed**(49 + codex [P1] 회귀 3). 엔드투엔드: 변경 전 week 윈도우 `verifiedCommits` **OMITTED** → 변경 후 **48**(`git log` 교차확인 일치). **/codex 적대 리뷰 2라운드**(gpt-5.5 high): 라운드1 [P1] 적출 — `_is_shallow()`가 git 에러를 `True`로 collapse → resolve된 repo가 shallow-probe 에러 시 poison 대신 silently skip(다른 valid repo 있으면 misleading partial 방출, 내 자신의 docstring "resolved repo error → poison"과 모순). 수정: `_is_shallow` **tri-state**(True=shallow skip / False=countable / None=에러→poison) + count_commits None 처리 + 회귀 3건(probe-error-poisons-even-with-valid-repo가 정확한 hole 못박음). [P2] collect.py:184 stale "poison the numerator" 주석 → "SKIP" 정정. 라운드2 재리뷰 **CLEAN**(no residual [P1]/[P2], resolution 경계 coherent: `_repo_root` 실패=skip, resolve 후 에러=poison, 신규 regression 0). privacy invariant 유지(cwd/SHA/email 미누출, codex 확인).
- ⚠️ 정직한 발견(owner 보고): 이 머신은 분자(48)는 풍부하나 비용($3,859.86/week, heavy Opus+cache)이 압도 → VES = 48/3859.86 ≈ 0.0124 → `fmtVes` toFixed(1) = **"0.0"**. "—" 문제는 해결됐으나 이 머신의 라이브 VES는 ~0.0으로 렌더됨(데모 시드 847÷$4.20=201.7과 cost 프로파일이 근본적으로 다름). **데이터 현실이지 코드 결함 아님** — cost 추정 정밀화는 본 사이클 범위 밖. 신규 업로드 시 리더보드에 0.0 등장 가능성을 owner가 인지하고 진입할지 결정 필요.

[S10 회고 — 2026-05-29]
- 무엇이 잘 됐나: 제로코드 프로브(`/tmp/ves_probe.py`)로 Option A(owner 재업로드)를 **코드 변경 전에** infeasible 판정 — 구조적 poison 3종(워크스페이스 부모/`~`/제거된 worktree) 상존을 증명해 헛수고 차단. codex 적대 리뷰가 **내 자신의 docstring과 모순되는 [P1]**을 적출 — "resolved repo error → poison" 무결성 경계를 `_is_shallow` 한 곳에서 빠뜨려 git 에러를 `True`(skip)로 collapse, valid repo 동반 시 misleading partial 방출. tri-state 수정 + "errored repo는 valid repo 동반 시에도 poison" 회귀로 정확히 못 박고, 라운드2 재리뷰로 CLEAN 확인.
- 다음엔 무엇을 바꿀까: 계약 무결성 규칙(unknown≠zero · resolved-error→poison)이 4개 헬퍼(`_repo_root`/`_is_shallow`/`_head_state`/`_repo_shas`)에 흩어져 한 곳이 규칙에서 이탈해도 단위테스트가 못 잡고 codex가 잡음 → 헬퍼별 tri-state 계약을 테스트로 강제하거나 resolution gate를 단일 지점으로 분리. 또 **VES≈0.0 발견은 분자만 보던 시야의 사각** — 분자 작업 시 분모(cost) 동반 sanity check(실제 `fmtVes` 렌더값 추정)를 평가기준에 포함해 "—는 고쳤지만 0.0이 뜬다"를 머지 전에 표면화할 것.

### 2026-05-29 VES 분모(AI-cost) 추정 정밀화 — 미상 Opus 마이너의 silent legacy 폴백 제거 [S0]

- 문제: VES = verifiedCommits ÷ AI-cost-USD. 직전 사이클(VES 분자 계약 완화) 정직한 발견 — 분자는 풍부(48)한데 cost($3,859.86/week)가 압도해 VES≈0.0. cost 추정 정밀화가 후속 과제로 격리됨. 진단: 가격표(`model-pricing.json`)에 현행 Opus 4.5–4.8이 **누락**돼 미상 Opus 마이너가 bare `claude-opus-4` 키($15)로 **high confidence** 폴백 → 실제 $5 모델을 3배 과대청구. 과대청구는 분모를 부풀려 VES를 **과소**표시(crediblity 리더보드에서 한 방향 왜곡). 구조적으로도 가격표 자체 주석의 의도("미상 마이너는 Estimated/low로 플래그")와 코드가 모순 — bare 키가 catch-all high로 동작.
- 버린 대안: (a) 데이터만 수정(현행 모델 키 추가) — 당장의 과대청구는 고치나 **다음** Opus 마이너(4.9 등)가 또 silent $15 high로 폴백하는 구조적 근원 미해결. owner가 Option B(데이터 + 구조적 근원) 명시 선택으로 기각. (b) 2-언어 매처 전면 재작성(정규식 기반 버전 파싱) — 과한 blast radius, TS/Python 패리티 회귀 위험. 이미 설계돼 있으나 미사용인 `-x` 와일드카드 메커니즘 활용으로 대체. (c) gpt-5.1-codex 가격 추정 주입 — OpenAI가 더 이상 단가 미공개 → 추정 주입은 정밀화 목표와 모순, **defer**(가짜 데이터 금지 원칙).
- 핵심 트레이드오프: 이미 설계됐으나 쓰이지 않던 **`-x` 현세대 와일드카드** 활용(저위험 테이블 변경) + 작은 confidence 조정. 매칭은 hyphen 경계 longest-prefix-wins 유지하되, (1) `-x` 키는 prefix를 strip해 현세대 전체를 catch하지만 **wildcard-only 매치는 'low'(Estimated)** 보고 → 미상 마이너는 sibling 단가를 high로 청구하지 않고 플래그. (2) 정확/장프리픽스 버전 키는 동률에서 wildcard를 이겨 'high' 유지. legacy 4.0/4.1은 명시 키($15)로 보존하고, **4.0 GA 스냅샷 `claude-opus-4-20250514`는 별도 핀**(그 id엔 `-0`가 없어 alias 키가 못 잡음 → 와일드카드가 $5로 오청구하는 걸 prefix 22>13 동률승으로 차단). bare `claude-opus-4`는 실재 id 아님(실 id는 스냅샷+`-0` alias) → $5/low 플래그(silent high 청구 절대 불가).
- 선택 이유: 과대청구(VES 과소표시) 0 + 구조적으로 미상 미래 마이너가 다시는 high-confidence legacy 단가로 폴백 불가. 3레이어 가격표(source JSON = 패키지 복사본 = codegen TS) 단일 진실 유지. 가짜 데이터 0(가격은 Anthropic 공식 문서·SDK model union으로 교차확인). 이 머신 실 데이터엔 영향 없음(이번 주는 명시 키 `claude-opus-4-7` 사용 → week cost $3,859.86 불변, 재업로드 불요) — 미래 미상 모델/타 사용자 업로드의 정확도를 위한 forward-fix.
- 강한 증거: 7파일 변경(가격표 2 JSON byte-identical + `parsers.ts`/`parsers.py` 매처 + `pricing.generated.ts` 재생성 + 양 언어 회귀 테스트). **pytest 28 · vitest 332/29파일 · tsc clean**. shasum으로 2 JSON 동일·codegen idempotent(드리프트 0) 확인. **/codex 적대 리뷰 2라운드**(gpt-5.5 xhigh): 라운드1 HIGH 2건 — (1) `claude-opus-4-20250514`(4.0 스냅샷)가 `-x` 와일드카드로 $5 low 매치(legacy $15 아님) → VES 부풀림, (2) bare `claude-opus-4` $5 low 강등(real alias면 3배 과소청구). 수정: 4.0 스냅샷 $15 명시 핀 + Anthropic SDK model.py로 bare `claude-opus-4`가 비실재 id임을 검증(실 id = 스냅샷 + `-0` alias, 둘 다 $15) + 양 언어 테스트로 못 박음(스냅샷→$15 high, bare→$5 low). 라운드2 재리뷰: **양 HIGH Resolved · 신규 HIGH/MED 0 · TS/Python 패리티 유지 · 머지 verdict approve**(codex가 독립적으로 테스트 재실행 + SDK 소스 교차확인).

[S10 회고 — 2026-05-29]
- 무엇이 잘 됐나: 직전 사이클의 정직한 "VES≈0.0" 발견을 placeholder가 아니라 **다음 사이클 진입점**으로 바로 전환 — 분모 정밀화를 독립 작업으로 격리해 분자 작업과 안 섞은 게 깔끔했다. 이미 설계됐으나 미사용이던 `-x` 와일드카드를 활용해 2-언어 매처 전면 재작성(고위험)을 피하고 테이블 변경 + 작은 confidence 조정(저위험)으로 구조적 근원을 해결. codex 적대 리뷰 라운드1이 내가 **"4.0은 deprecated니까 넘어가자"로 손쉽게 넘기려던 4.0 GA 스냅샷 과소청구(HIGH)**를 적출 → Anthropic SDK `model.py`로 실 id 카탈로그(스냅샷 + `-0` alias, bare 키는 비실재)를 교차확인하고 $15 핀 + 양 언어 테스트로 못 박았다. 머지 후 라이브 4표면(API/StatusBar Top-VES/리더보드 컬럼/Ticker) 전부 `0.0(@chongwon83)`로 일치 확인 — 분모 fix가 이번 주 숫자를 안 건드림(explicit 키)을 실측으로 검증.
- 다음엔 무엇을 바꿀까: **돈 인접 코드(가격/청구)에서 undercharge 경로를 "아마 안 쓰일 것"으로 손쉽게 기각하지 말 것** — 실 모델-id 카탈로그(SDK 소스)로 검증이 기본. 또 3레이어 가격표 동기화(source JSON = 패키지 복사본 = codegen TS)가 **수동·취약** — 복사본 drift나 stale codegen이 silent하게 머지될 수 있다. CI/pre-commit에 ① 두 JSON shasum 동일 ② `codegen-pricing.mjs` 재실행 후 `git diff` 비어있음(드리프트 0) 가드를 추가해 이 사이클의 수동 검증을 자동화할 것. 잔여 정확도 갭: `gpt-5.1-codex` 단가(OpenAI 미공개)는 defer 상태 — codex 측 분모는 그만큼 미반영.

### 2026-05-29 가격표 3레이어 동기화 CI 가드 — 수동 검증 자동화 [S0 → 완료]

- 문제: 바로 위 사이클(VES 분모 정밀화) S10 회고의 액션아이템 종결. 3레이어 가격표 동기화(source `model-pricing.json` = 패키지 복사본 = codegen `pricing.generated.ts`)가 **수동·취약** — 복사본 drift나 stale codegen이 silent하게 머지되면 Python·브라우저 콜렉터가 같은 모델을 다르게 청구 → VES 분모 왜곡. 그간 shasum + codegen 재실행 + `git diff`를 손으로 검증.
- 버린 대안: (a) codegen을 render 함수로 export해 비교 — `.mjs`를 `.ts` 테스트에 import하는 tsc 해상도 리스크 + codegen 자체를 건드리는 위험으로 기각. (b) shell CI 스텝 — 로컬 미실행 + 워크플로 편집 필요로 기각. 선택: **vitest 테스트 1파일**(`__tests__/pricing-table-sync.test.ts`)로 content-equivalence 검증 → 기존 `npm test`(ci.yml `test` 잡)에 자동 편입, 워크플로 편집·codegen 변경 0, 로컬에서도 동일 실행.
- 핵심 트레이드오프: 두 실패모드를 한 파일에서 잡되 중복 최소화. FM1 = source JSON ≠ 패키지 복사본(raw byte 비교 → 공백·순서 변화까지). FM2 = `pricing.generated.ts` stale(`MODEL_PRICING`/`PRICING_AS_OF` deep-equal vs source JSON). 테스트의 `stripMeta` 헬퍼는 codegen `pickPricing`을 미러(`_`-prefix 메타키 strip, `_default`만 유지) — ~3줄 중복이 codegen 전체 import보다 안전. ⚠️ codegen 필터 로직 변경 시 `stripMeta`도 동기화 필요(파일 주석에 명시).
- 강한 증거: 양 FM에 드리프트 **주입**(복사본에 `\n` 추가 / generated TS 단가 변조) → 각 expect가 actionable 메시지(re-copy / re-run codegen)로 **실패** 확인 후 복원(working tree clean) → false-guard 아님 입증. 로컬 vitest **334/334·30파일**(이전 332/29 → +2 tests/+1 file)·tsc clean. main `919fb5f` 머지(클린 FF) + 3 CI green(CI/parity/security). 라이브 CI 로그(run `26625737469`, `test` 잡 "Unit tests" 스텝)에 `✓ __tests__/pricing-table-sync.test.ts (2 tests)` + `Test Files 30 passed (30)` + `Tests 334 passed (334)` 직접 확인 — 잡 레벨 green 넘어 **파일 단위 CI 실행 증명**.
- 잔여: `gpt-5.1-codex` 단가 defer 유지(OpenAI 미공개, 상위 사이클에서 이월) — 가드는 단가 정확성이 아니라 3레이어 **동기화**만 강제하므로 무관.

### 2026-05-29 VES 노트 신뢰 단서 1줄 + dead spec link 수정 — 헤드라인 지표 정직성 마감 [S0 → 완료]

- 문제: 라이브 VES 표면(`BurnIndexSection`)에 두 정직성 갭. (1) VES 공식 노트가 "verified fixes ÷ AI cost"만 설명하고 *무엇이 fixes를 verified로 만드는가*를 안 밝혀 분자가 신뢰 기반(self-entered)으로 읽힐 여지. (2) 인접 트러스트 카피가 수집 스펙을 "public and auditable"이라 주장하는데 "Read the full collection spec →" 링크가 `href="#"` dead link — 검증 가능 주장과 모순.
- 버린 대안: (1) 노트 raw Option A 문구 "…on-device — not self-entered" — owner가 초안 승인했으나 plan-design-review·codex 둘 다 **독립적으로** 가시적 "Self-reported" 증거 티어와 표면 충돌(절대 부정문)을 적출 → 기각하고 합성 문구로 ship(owner에 발산 사실·이유 투명 보고). (2) dead link를 임의 non-`#` href로 채우기 — "auditable" 주장 미충족 → 실제 스펙 타깃을 Explore로 조사 후 결정으로 대체.
- 핵심 트레이드오프: 노트는 절대 부정("never self-reported") 회피하고 *출처*만 명시 — `Verified fixes come from your own git commits, counted on-device.` (티어 사다리 Provider>Device>Estimated>Self-reported와 양립, codex가 짚은 HEAD-only/own-email/when-available over-claim도 회피). 링크는 기존 외부링크 컨벤션(`COLLECTOR_REPO_URL` 재사용 + `target=_blank rel=noopener noreferrer`)으로 실제 public 스펙(`/tree/main/tools/usage-poc`: `burn-summary.schema.json` + collector 소스 + README) 지목.
- 선택 이유: 분자 신뢰성 단서가 가시 증거 티어와 모순 0 + "auditable" 주장이 실제 클릭 가능한 스펙으로 뒷받침됨. 3스킬 협업(plan-design-review 0-10 비례 적용 / frontend-design microcopy / codex 적대 검증)으로 한 줄 카피에 과한 절차(7-pass/목업) 안 씌우되 두 독립 over-claim 발견을 한 정확한 문장으로 합성. visual baseline 영향 0(노트·트러스트 블록은 모바일/데스크톱 clip 하단 렌더).
- 강한 증거: 2커밋 FF 머지 — `afd1638`(VES 노트) + `1ef0fe5`(dead link). tsc clean · vitest green · SSR 렌더 확인 · Claude-in-Chrome 로컬 검증 + /codex 교차. 양 GitHub URL HTTP 200(스펙 public 실재). 라이브 실측(`curl coconutlabs.xyz`): 새 노트 문장 1건 · real spec href 1건 · 페이지 전체 `href="#"` **0건** 확인. 3 CI green(CI/parity/security) 각 푸시.
- 함정 기록(TIL): 라이브 검증 1차에서 `grep PATTERN | head -1 && echo ⚠️ || echo ✅` 구조가 **거짓 dead-link 경보**를 냄 — 파이프라인 종료코드가 마지막 명령 `head`(항상 exit 0)로 결정돼 grep 매칭 여부와 무관하게 `&&` 분기가 발화. `grep -oE` 정식 재검으로 매칭 0건 확정. → 조건 분기용 grep에 `| head`를 끼우지 말 것(`grep -q` 또는 `${PIPESTATUS[0]}`로 grep 종료코드 직접 검사).

### 2026-05-29 랜딩 VES 디엠퍼시스 + WCAG teal 접근성 + 폴리시 (Topic 1+2) [S0 → 완료]

- 문제: `/refine` 리뷰가 중심 역설 적출 — owner는 VES를 *더* 강조하길 원했으나 VES(=verified fixes ÷ AI cost)는 이미 3곳+ 구조적 헤드라인인데 분자(verified fixes)가 현실 ≈0이라 모든 라이브 행이 `0.0`/`—` 렌더. "데이터를 믿어라"가 핏치인 제품에서 빈 지표를 헤드라인하면 신뢰 훼손. 동시에 브랜드 그린 `#00D084`를 흰 배경 텍스트로 쓰면 WCAG AA 미달(~2:1).
- 버린 대안: (a) VES를 빈 채로 더 키우기 — 역설 심화, 기각. (b) 분자를 채우는 백엔드 작업 — 이번 사이클 범위 밖(백엔드 무변경 제약). (c) 그린 전면 교체 — fill/border(다크 텍스트 위)는 대비 충분하므로 **텍스트 용도만** teal 분리. (d) 모바일에서 VES 표시 유지 — 공간 제약 + 대부분 Pending이라 기각, 단 정렬은 VES 기준 유지(뷰포트 간 랭크 일관성).
- 핵심 트레이드오프: VES를 *지우지 않고* 임계(≥2 nonzero VES, `hasEnoughVes`/`VES_REVEAL_THRESHOLD=2`)까지 **숨김** — 컬럼·StatusBar Top-VES·Ticker VES 단일 게이트. 숨김 상태 기본 정렬은 Tokens desc(빈 VES 정렬 재현 방지), 노출 시 VES desc. null/zero VES는 "Pending"(`0.0`/`—` 금지). 텍스트 그린→teal `--accent-text`=`#0F766E`(~5.5:1), fill/border 그린은 보존. Hero 카피 drag→costs. Ticker 자체 폴링 제거(LandingApp 단일 소스).
- 선택 이유: 빈 지표 헤드라인 제거 + 대비 위반 해소 + 백엔드 0 변경. 실데이터 도착 시 게이트 자동 노출. codex 적대 리뷰가 노출 상태 엣지 2건 적출 → 머지 전 수정.
- 강한 증거: 1차 — tsc clean · vitest green · 18 e2e(gate/sort/filter 양 VES 상태) · Claude-in-Chrome 양 상태 검증 + /codex 교차. visual rebaseline(hero teal+카피, Linux PNG mobile-375만 diff, `9e9f953`). 머지 `02f98b6`(rebase, 선형 히스토리) · main 5 CI green(test/e2e/visual/parity/security) · Vercel Production=`02f98b6` · 라이브 `www.coconutlabs.xyz` 200에 새 hero 카피(`your AI coding actually`)+`hero-accent">costs` 실측.
- codex 후속(노출 상태 잠복 엣지, 머지 전 `02f98b6`로 수정): (1) `ves===0`이 "Pending"인데 comparator가 숫자 0으로 정렬 → asc에서 real score 위로 부상. (3) `userTouchedRef`가 ves→tokens 기본키 전환을 차단 → 게이트 오프된 VES에 정렬 latch. 수정: 순수 `compareBy` 추출(ves 0→null 취급, 양방향 Pending 하단) + `vesColumnShown` 가드(숨김 시 fallback) + 단위테스트 4(`__tests__/column-sort-compare.test.ts`). codex 발견 (2) 모바일 VES 숨김+VES 정렬은 **의도**(뷰포트 간 랭크 일관성 + 항상 보이는 rank 컬럼이 순서 전달), aria-sort 갭만 a11y 폴리시로 defer.

[S10 회고 — 2026-05-29]
- 무엇이 잘 됐나: "VES를 더 강조"라는 owner 직관을 그대로 따르지 않고 `/refine`으로 *빈 지표를 헤드라인하는 역설*을 먼저 표면화 → 강조 대신 **임계 게이트 숨김**으로 방향 전환(직관 반대지만 근거로 설득). Topic 1(a11y/폴리시)+2(VES 디엠퍼시스)를 백엔드 무변경으로 묶고 Topic 3(comms)는 별도 plan으로 격리. codex 적대 리뷰가 노출 상태 정렬 엣지 2건(zero-Pending asc 부상 / 게이트오프 latch)을 적출 — 라이브(showVes=false)엔 잠복이나 실데이터 도착 시 발현할 forward-bug를 머지 전에 순수 comparator 추출 + 가드로 못 박음(node-env vitest로 DOM 없이 검증).
- 다음엔 무엇을 바꿀까: **조건부 렌더 컬럼은 정렬 키 유효성과 항상 동반 검증** — 컬럼 게이트(showVes)와 정렬 상태(sortKey)가 따로 놀아 codex가 잡은 latch 버그가 났다. 게이트되는 컬럼 추가 시 "그 키로 정렬 중이면 fallback" 규칙을 훅 계약 테스트로 강제할 것. 노출 상태(≥2 nonzero VES)는 라이브에 아직 없어 e2e seed로만 커버 — 실데이터 첫 도착 시 노출 상태 라이브 검증 1회 의무(모바일 aria-sort 갭 포함). VES 분자(verified fixes)가 여전히 ≈0이라 게이트는 당분간 항상 숨김 — 분자를 채우는 작업이 다음 큰 레버.

### 2026-05-30 Topic 3 (comms) — 인바운드 contact + 옵션 이메일 옵트인 + PII 게이트 사인오프 [S0 → 완료]

- 문제: `/refine`(2026-05-29)이 **양방향 comms 부재**를 적출 — 데이터를 남긴 유저에게 알림 보낼 채널도, 그들이 연락할 채널도 없음(Threads는 followers≠데이터 제공자·broadcast·미소유라 불충분). 인바운드(footer contact)·아웃바운드(스캔 후 옵션 이메일 옵트인) 두 채널을 추가하고 legacy dead "Notify me" 어포던스를 정리. **신규 PII(이메일) 수집 = privacy 포지셔닝과 충돌 가능** → 빌드 전 owner 사인오프 필수(privacy gate).
- 버린 대안: (a) 이메일 없이 Threads-only — `/refine`이 미소유·broadcast-only로 불충분 판정. (b) 사이트 전역 "Zero / only aggregated numbers leave your device" 카피를 약화 — 그 약속은 *코드/프롬프트/경로*에 관한 것이라 여전히 문자 그대로 참 → 약화는 과잉수정, 기각. (c) 이메일↔핸들 비연결 저장 — 타깃 알림(랭크 변동) 불가 → 연결 저장하되 수집 시점에 명시(carve-out). (d) Topic 3을 Topic 1+2와 한 PR로 — 백엔드 신설(EmailStore+엔드포인트+토큰 kind)이라 별도 plan으로 격리(직전 사이클에서 결정).
- 핵심 트레이드오프(privacy gate **사인오프**): "**Separate opt-in + carve-out**" — 이메일은 리더보드와 **별도 store**(`EmailStore`, key `burn:emails:v1`)에 저장, 절대 공유/판매 안 함, 제품 업데이트 용도만. 사이트 전역 "Zero" 약속은 **불변**(코드/프롬프트/경로 대상 → 여전히 참). 이메일은 **유일한 옵션·명시동의 예외**이며 수집 시점(`PostUploadSurvey` step 2, 체크박스 동의)에 framing. 동의는 **서버 강제**(`/api/emails`가 `consent !== true` → 400, UI만 아님). 토큰 kind `emails`는 기존 `COLLECTOR_HMAC_SECRET` 재사용(신규 env 0).
- 선택 이유: 인바운드+아웃바운드 comms 신설하면서 privacy 내러티브 무손상 + legacy UX 불일치 제거. EmailStore는 `BurnStore`가 리더보드 전용(namespace 비제네릭)이라 **병렬 store + `getEmailStore()` 팩토리**로 신설(File/Redis/Memory trio + env 우선순위 미러). 토큰은 kind 제네릭이라 union(line 21) + parseToken allowlist(line 68) + 발행 allowlist 3곳만 편집.
- 강한 증거: 22파일(+1063/−53). **tsc clean · vitest 386/34파일**(신규 email 3파일: validation/store/route-token-integration — 실발행 토큰을 실 route로 통과시켜 spec drift 적출, 401×4/400×3/200×2 + parseToken emails 라운드트립) · `npm run build`(`/api/emails` dynamic 등록 + legacy tree-shake 유지) · **playwright 58 passed**(신규 `topic3-comms.spec.ts` 3: footer mailto / 옵트인 게이트+payload `{email,handle,consent:true}` / skip+decline no-POST — FSA happy-path가 PostUploadSurvey 렌더하는 유일 경로라 그 harness 차용).
- **/codex 적대 리뷰**(gpt-5.5 high, 1M tokens) 4건 + owner 판정:
  - **[P1] 비인증 PII write**(anyone가 `emails` 토큰 발행 → 임의 email/handle POST, consent는 client-asserted, 업로드/세션/행 바인딩 없음). 검증: **기존 collector 신뢰모델 그대로**(burnindex/telemetry도 비인증 write) — Topic 3 회귀 아님. 증폭 없음(**아웃바운드 mailer 부재** → confirmation 이메일 안 보냄, private store에 landing + 정규화 dedupe). 진짜 fix(double opt-in)는 mailer 필요 = MVP 범위 밖. → **owner: Ship MVP-accepted**(문서화된 MVP 한계, double opt-in은 mailer 도입 시).
  - **[P1] nonce GET-then-DEL race**(`token.ts:122-131`, 주석은 "atomic"이라 거짓 표기). 검증: **pre-existing**(내 diff는 TokenKind union+allowlist 2줄만, nonce 로직 무변경) + burnindex/telemetry 공통. email 경로엔 **benign**(EmailStore 정규화 dedupe → 동일 토큰 동일 이메일 중복 write는 no-op, client는 호출당 fresh 토큰). → **flag only**, 본 PR 미수정(공유 모듈 Redis GETDEL/Lua는 별도 작업 + 교차 테스트).
  - **[P2] privacy 카피 절대성**: 옵트인이 "stored separately from the leaderboard"라면서 **핸들 연결 미고지**. → **수정함**(`PostUploadSurvey.tsx:111-114`): "we store it with your handle (so we can find your row), kept separate from the public leaderboard and never shared or sold." (`data.ts:185` "Only … leave your device"는 스캔 플로우 스코프 → owner 기존 사인오프 유지, 미변경.)
  - **[P2] rate-limit silent fail**(이메일 토큰이 5/min 한도의 happy-path 5번째 발행 → 리트라이/추가 telemetry 시 429 → `subscribeEmail` silent drop → `onDone` 호출 → UI는 성공 암시). 검증: 사실(가장 깨끗한 경로에서 정확히 경계). → **owner: Accept for MVP**(telemetry와 fire-and-forget 패리티, 저트래픽이라 경계 희소 도달). 알려진 한계.
- 정직한 한계(owner 인지·수용): (1) 비인증 PII write — 임의 3자가 store 오염/비동의 이메일 seed 가능(증폭 없는 private-store 오염). (2) opt-in silent-drop — 명시 액션인데 429 시 조용히 no-op. 둘 다 MVP 수용, mailer 도입 시 재방문.
- 임시 연락처: `CONTACT_EMAIL = chongwon5026@gmail.com`(`lib/data.ts` 단일 상수 → coconutlabs.xyz alias 1줄 스왑). visual baseline 영향 0(footer는 above-fold clip 하단).

### 2026-05-30 Part B — 브라우저 VES 분자 수집(isomorphic-git/FSA) + 분자 precedence merge [S0 → 완료]

- 문제: VES = verifiedCommits ÷ AI-cost. 브라우저(FSA) 업로드는 git을 못 돌려 `verifiedCommits`를 **항상 누락** → VES null("Pending"). 정확한 VES가 CLI 임포트에만 의존하는 fragile 상태. 동시에 탐색으로 잠복 버그 확인: 3개 store 전부 handle 통째 교체(`[entry, ...prev.filter(handle≠)]`)인데 `buildImportedEntry`가 `env.verifiedCommits → entry.fixes`를 복사하므로, **numerator 없는 재업로드 하나가 기존 CLI 커밋수(예 153)를 지움** — 과거 "VES 컬럼 재업로드 후 사라짐" 사건의 한 갈래.
- 버린 대안(owner+codex 합의 4결정): (D1) 분자수집·precedence를 두 PR로 분리 — merge 로직이 분리 여부와 무관하게 동일해 **한 PR**(`feat/ves-browser-numerator`)로. (D2) Redis 분자 병합을 cjson-in-Lua / 사이드카 — Upstash cjson 미보장(web search) + 빈배열 `[]→{}`·float 재인코딩·nil 소실·레거시 행 함정 부활 → 기각, **TS read-merge-write**(병합된 blob을 기존 원자 Lua에 그대로, Lua 재작성 0). (D3) Python CLI 변경 — blast radius 축소 위해 **미변경**, source 미명시는 서버가 `"cli"` 기본(하위호환). (D4) `pako` 직접 추가 — transitive로 충분 → `isomorphic-git`만 해시핀/`--ignore-scripts`/owner 실행.
- 핵심 트레이드오프: 분자 병합만 비원자(HGET-then-EVAL)로 완화하되 leaderboard+history 페어링 원자성(기존 Lua)은 보존 — 솔로·저트래픽이라 read-before-write 레이스 사실상 0. 3 store(file/memory/redis)가 **공유 pure 헬퍼 `mergeNumerator`**(코드↔도메인 1:1). **최대 트랩(codex)**: 레거시 저장 CLI 행은 `fixes`는 있고 `fixesSource`는 없음(필드가 오늘 없으니) → `rankSource`의 `else→2`로 stored·incoming 양쪽에서 CLI(2) 취급해야 첫 browser-fsa 업로드가 레거시 CLI를 못 덮음. precedence rank: cli=2 > browser-fsa=1 > absent=0. 클라: raw cwd / `selectedAuthorEmails` / FSA 핸들 경로는 **절대 envelope·POST·로그 미진입**(로컬 사이드채널 Map, gitcount 입력 후 폐기). gitcount는 `gitcount.py` 정확 포팅(author 정확일치·반열림 `[since,until)`·repo+SHA 전역 dedup·shallow skip·resolved-error poison) + lazy `await import("isomorphic-git")`(기본 번들 미오염).
- 선택 이유: 브라우저 업로드도 분자를 실어 VES를 채울 수 있고, 나중 browser/absent 업로드가 CLI 커밋수를 **못 덮고 못 낮춤** + 잠복 clobber 버그 동시 해소. `computeVes()` raw ratio 미변경(Part A `fmtVes` per-$1k 표시 유지).
- 강한 증거: 24파일(+2724/−71). local tsc clean · **vitest 465/465·39파일**(신규 burn-merge-numerator / burn-numerator-precedence ×3 store / burn-rawcwd-sidechannel / burn-repogroups / gitcount — gitcount은 실제 git repo over node:fs로 카운팅·dedup·author제외·반열림 경계·genuine-zero·unborn HEAD·headState/isShallow tri-state·resolved-error poison 커버) · playwright FSA spec 3/3(신규 "Step 3 numerator grant button" 렌더 e2e). **/codex diff 리뷰 2패스**(resume 019e76fc, read-only): MAJOR 1건 — *stale/desync numerator*(카운트 후 author 토글/재grant/recount 실패 시 stale verifiedCommits가 POST 잔류; 1차 수정 후 mid-count 레이스 잔존 — 카운팅 중 토글이 strip해도 captured-stale-author 카운트가 resolve돼 reattach). 수정: `clearCount()`(strip+세대 bump) 업프런트 + `countGenRef` 세대 가드(in-flight 카운트 superseded면 reattach drop) + 카운팅 중 author chips `disabled`, **success만 reattach**. 라운드2 재리뷰 PASS(selection-change / re-grant / scan-reset / sequential-recount 전부).
- 머지: PR #42 squash → main `e434c5e`(`gh api -X PUT /merge`, worktree 충돌 회피; `gh pr merge` 금지). 솔로라 리뷰 요청 없이. 커밋은 `issues/*.png` churn(3 del + 1 untracked) 제외 + package.json/lock(isomorphic-git@1.38.3 exact-pin) 동반. 머지 후 5 CI green(test/visual/e2e/parity/security) — **visual job PASS = rebaseline 불필요(실증)**. 프로덕션 스모크(www 직접, apex 307): www home 200 + /api/burnindex 200 valid JSON(@mpapa 레거시 행이 새 hydrate/mergeNumerator 경로 무사 통과, fixes:15) + 404 대조 404. SSR/API 무회귀.
- 잔여(블로커 아님): 자동화 불가 1조각 — 네이티브 폴더 피커로 **실제 OS git repo 폴더** 선택 후 createFsaFs 라이브 카운트 1회(피커가 OS레벨이라 e2e 불가; 코어는 gitcount.test.ts가 실제 git repo로 커버). owner 브라우저 grant→chips→count 1회 확인 권장.

[S10 회고 — 2026-05-30]
- 무엇이 잘 됐나: 분자 수집(신기능)과 precedence merge(잠복 clobber 버그 fix)를 한 작업으로 묶되 **서버(분자 보호)를 먼저 그린**으로 만들고 클라(브라우저 수집)를 뒤에 둬 — 클라 없이도 가치 있는 순서. codex가 **설계(plan) 리뷰**에서 cjson-in-Lua를 기각시키고(Upstash 미보장 + 빈배열 함정) **레거시 행 rank 트랩**(fixes-present·source-absent를 CLI로 취급 안 하면 첫 browser 업로드가 레거시 CLI 덮음)을 미리 짚어 `rankSource else→2`로 반영 — diff가 아니라 plan에서 잡은 게 컸다. **diff 리뷰 2패스**가 1차 수정으로 안 닫힌 mid-count 레이스(author chips 활성 중 토글)를 적출 → 세대 토큰(`countGenRef`)으로 정확히 못 박음. Evidence 게이트는 tsc+vitest 465+playwright+5 CI+www 스모크까지 한 번도 안 건너뜀.
- 다음엔 무엇을 바꿀까: (1) **표시 표면 인벤토리 선행** — 이번 주 VES 디스플레이 작업이 StatusBar/Ticker/legacy 3곳에서 같은날 follow-up fix를 냈다. 분자/표시 변경 전 렌더 사이트 1회 grep+나열을 평가기준에 넣을 것. (2) **decision-log 사이클당 1엔트리로 통합** — `decision-log.md`가 이번 주 hotspot #1(28 touches). 스텝별 append 대신 사이클 종료 시 1회 통합 기록(이 엔트리가 그 시도). (3) **deep block 보호** — 40세션 중 26개가 micro(<20분), 실작업은 deep 4개에 집중. 잔여 open loop 1건: 네이티브 피커 라이브 카운트 1회(실데이터 end-to-end 검증)가 다음 진입 시 첫 액션.
